<div class="category_right" style="border:1px dotted red;border-radius:10px;">
	...
	<div  class="category_content_video" style="margin-top:1px;">
<?php
...
// 如果$cats不为空，说明有子分类
    ...
    foreach($cats as $the_cat){  //先遍历出所有子分类
        ...
        if(!empty($posts)){ //如果分类下有文章列表，列出/* 先用item cat_item 的h2标签列出分类名字及链接,再用ul+foreach列出文章列表
            echo '
            <div class="category_content_video" style="min-height:30px;height:100%;padding-bottom:20px;">
                ...
                <ul>';
                    foreach($posts as $post){
                        echo '<li style="background:url( http://test.pldj.cn/wp-content/themes/leanhorse/css/video.png) no-repeat left;"><span class="alignright">'.mysql2date('Y-m-d', $post->post_date).'</span>
                        <a title="'.$post->post_title.'" href="'.get_permalink($post->ID).'" target="_blank">'.$post->post_title.'</a></li>';
                    }
                echo '</ul>
            </div>';
        }
