...
<!--     <div class="list_header">
	...
	<div  class="category_content" style="margin-top:1px;">
<?php
...
<div class="category_content">
    <ul>
        ...
        <li><span ><?php the_date_xml(); ?></span><a title="<?php the_title(); ?>" href="<?php the_permalink(); ?>" target="_blank"><?php echo mb_strimwidth(get_the_title(),0,80,'...'); ?></a></li>
...
// 如果$cats不为空，说明有子分类
    ...
    foreach($cats as $the_cat){  //先遍历出所有子分类
        ...
        if(!empty($posts)){ //如果分类下有文章列表，列出/* 先用item cat_item 的h2标签列出分类名字及链接,再用ul+foreach列出文章列表
            echo '
            <div class="category_content" style="min-height:30px;height:100%;padding-bottom:20px;">
                ...
                <ul>';
                    foreach($posts as $post){
                        ...
                        <a title="'.$post->post_title.'" href="'.get_permalink($post->ID).'" target="_blank">'.mb_strimwidth(($post->post_title),0,76,'...').'</a></li>';
                    }
                echo '</ul>
            </div>';
        }
